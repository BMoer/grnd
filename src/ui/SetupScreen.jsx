...
export default function SetupScreen() {
  const { classConfig, assumptions, updateAssumption, startGame } = useGameStore();
      ...
      <div className="w-full max-w-2xl">
            ...
            {Object.entries(presets).map(([key, preset]) => {
              ...
              return (
                <button
                  ...
                  style={{
                    background: isSelected ? 'var(--color-surface)' : 'var(--color-raised)',
                    border: `2px solid ${isSelected ? preset.color : 'var(--color-border)'}`,
                    outline: isSelected ? `1px solid ${preset.color}40` : 'none',
                  }}
                >
                  <div className="flex items-center gap-2 mb-1.5">
                    <span className="text-base" style={{ color: preset.color }}>{preset.icon}</span>
                    <span className="text-sm font-bold" style={{ fontFamily: 'var(--font-display)', color: isSelected ? preset.color : 'var(--color-text)' }}>
                      {preset.label}
                    </span>
                  </div>
                  ...
                  <div className="flex gap-4">
                    <div>
                      <div className="text-[9px] uppercase tracking-widest mb-1 font-medium" style={{ color: 'var(--color-growth)', fontFamily: 'var(--font-mono)' }}>
                        Pros
                      </div>
                      {preset.pros.map((p, i) => (
                        <div key={i} className="text-[10px] leading-snug" style={{ color: 'var(--color-text-secondary)', fontFamily: 'var(--font-mono)' }}>
                          + {p}
                        </div>
                      ))}
                    </div>
                    <div>
                      <div className="text-[9px] uppercase tracking-widest mb-1 font-medium" style={{ color: 'var(--color-danger)', fontFamily: 'var(--font-mono)' }}>
                        Cons
                      </div>
                      {preset.cons.map((c, i) => (
                        <div key={i} className="text-[10px] leading-snug" style={{ color: 'var(--color-text-secondary)', fontFamily: 'var(--font-mono)' }}>
                  ...
                  {/* Show key numbers */}
                  {isSelected && (
                    <div className="grid grid-cols-3 gap-2 mt-3 pt-3" style={{ borderTop: '1px solid var(--color-border)' }}>
                      <MiniStat label="Price" value={`€${preset.assumptions.price}/mo`} />
                      <MiniStat label="Churn" value={`${preset.assumptions.churnRate}%`} />
                      <MiniStat label="Pipeline" value={`${preset.assumptions.pipelineGrowth}/mo`} />
                    </div>
                  )}
                </button>
              );
        ...
        <div className="p-4 rounded mb-6" style={{ background: 'var(--color-surface)', border: '1px solid var(--color-border)' }}>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            <ForecastStat label="M6 MRR" value={`€${month6.totalMRR?.toLocaleString('en-US') ?? '0'}`} />
            <ForecastStat label="M12 MRR" value={`€${month12.totalMRR?.toLocaleString('en-US') ?? '0'}`} />
            <ForecastStat label="M12 Customers" value={`${month12.customers ?? 0}`} />
            <ForecastStat label="M12 Cash" value={`€${month12.cash?.toLocaleString('en-US') ?? '0'}`} sub={month12.cash < 0 ? 'Bankrupt before M12' : undefined} />
          </div>
        </div>
...
function MiniStat({ label, value }) {
  return (
    <div className="text-center">
      <div className="text-[8px] uppercase tracking-widest" style={{ color: 'var(--color-text-muted)', fontFamily: 'var(--font-mono)' }}>{label}</div>
      <div className="text-[11px] font-bold tabular-nums" style={{ fontFamily: 'var(--font-mono)', color: 'var(--color-text-secondary)' }}>{value}</div>
    </div>
  );
}
